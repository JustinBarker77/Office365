...
function Export-MSOLUserLicenceBreakdown {
    ...
    if ($FilterCSVPath)
    {
        ...
        $allLicensedUsers = Get-MsolUser -All | Where-Object { $_.isLicensed -eq $true -and $_.UserPrincipalName -in $filteredUsers.UserPrincipalName }
    }
    else
    {
    ...
    foreach ($license in $licenseType)
    {
        ...
        foreach ($user in $users)
        {
            ...
            $userHashTable = @{
                ...
                DirectorySynced   = [bool]$user.LastDirSyncTime
            ...
            foreach ($row in $($thislicense.ServiceStatus))
            {
                $serviceName = $(
                    ...
                    {
                        Get-LicenceTranslation -SKU $($row.ServicePlan.ServiceName) -LicenceLevel Component
                    }
                )
            ...
            foreach ($status in 'Success', 'Pending', 'Disabled')
            {
                ...
                foreach ($component in $thislicense.ServiceStatus)
                {
                    $serviceName = $(
                        ...
                        {
                            Get-LicenceTranslation -SKU $($component.ServicePlan.ServiceName) -LicenceLevel Component
                        }
                    )
